...
erpnext.selling.SalesOrderController = erpnext.selling.SellingController.extend({
	...
	make_purchase_order_cpce: function(){
		let pending_items = this.frm.doc.items.some((item) =>{
			let pending_qty = flt(item.stock_qty) - flt(item.ordered_qty);
			return pending_qty > 0;
		})
		...
		var me = this;
		var dialog = new frappe.ui.Dialog({
			title: __("Select Items for CPCE Purchase Order"),
			fields: [
			...
			primary_action_label: 'Create CPCE Purchase Order',
			primary_action (args) {
				if (!args) return;
				...
				var method = "make_purchase_order_cpce"
				return frappe.call({
					method: "erpnext.selling.doctype.sales_order.sales_order." + method,
					freeze: true,
					freeze_message: __("Creating CPCE Purchase Order ..."),
					args: {
						"source_name": me.frm.doc.name,
						"selected_items": selected_items
					},
