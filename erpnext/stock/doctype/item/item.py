...
def validate_end_of_life(item_code, end_of_life=None, disabled=None, invalid_after=None, verbose=1):
	if (not end_of_life) or (disabled is None) or (not invalid_after):
		end_of_life, disabled, invalid_after = frappe.db.get_value("Item", item_code, ["end_of_life", "disabled", "invalid_after"])

	if end_of_life and end_of_life != "0000-00-00" and getdate(end_of_life) <= now_datetime().date():
		msg = _("Item {0} has reached its end of life on {1}").format(item_code, formatdate(end_of_life))
		_msgprint(msg, verbose)

	if invalid_after and invalid_after != "0000-00-00" and getdate(invalid_after) <= now_datetime().date():
		msg = _("Item {0} is invalid since {1}").format(item_code, formatdate(invalid_after))
		_msgprint(msg, verbose)
