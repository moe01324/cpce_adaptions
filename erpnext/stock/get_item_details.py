...
def get_item_price_Branch(args, item_code, ignore_party=False):
	"""
		Get name, price_list_rate from Item Price based on conditions
			Check if the desired qty is within the increment of the packing list.
		:param args: dict (or frappe._dict) with mandatory fields price_list, uom
...
def get_price_list_rate_for_Branch(args, item_code):
	"""
		:param customer: link to Customer DocType
		:param supplier: link to Supplier DocType
		:param price_list: str (Standard Buying or Standard Selling)
	...
	item_price_data = 0
	price_list_rate = get_item_price_Branch(item_price_args, item_code)
	if price_list_rate:
		desired_qty = args.get("qty")
		if desired_qty and check_packing_list(price_list_rate[0][0], desired_qty, item_code):
			item_price_data = price_list_rate
		...
		general_price_list_rate = get_item_price_Branch(item_price_args, item_code,
			ignore_party=args.get("ignore_party"))

		if not general_price_list_rate and args.get("uom") != args.get("stock_uom"):
			item_price_args["uom"] = args.get("stock_uom")
			general_price_list_rate = get_item_price_Branch(item_price_args, item_code, ignore_party=args.get("ignore_party"))
	...
	if item_price_data:
		if item_price_data[0][2] == args.get("uom"):
			return_val = item_price_data[0][1]
		elif not args.get('price_list_uom_dependant'):
			return_val = flt(item_price_data[0][1] * flt(args.get("conversion_factor", 1)))
		else:
			return_val = item_price_data[0][1]
	return return_val
